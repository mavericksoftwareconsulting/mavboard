  ...
  syncState(evt) {
    ...
    switch (evt) {
      ...
      case 'event:volumeChanged':
        ...
        window.music.mopidy.mixer.getVolume().then(v => {
          document.getElementById('music-volume-control').value = v;
        });
        break;
...
/**
 * function called whenver the track has started playing
 * to update the cover art shown on the center screen.
 *
 * @param {string} trackUri is the specific uri for the track we want the image for.
 * @param {Array} imgs is a list of images we want to sort through for the track uri.
 */
...
/**
 * This function runs through an interval, and moves the current time bar
 * closer to the total time.
 */
